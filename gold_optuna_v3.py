...
================================================================================
  ...
  Strategy : Multi-Timeframe Analysis (MTFA)
             + DNN โ— XGBoost  "Strict Veto" Ensemble
  ...
  โ”  ยง6  TradingSimulator    โ€” macro filter โ’ veto gate โ’ trailing stop      โ”
...
class Config:
    ...
    # โ”€โ”€ Ensemble veto gate  [OPTUNA] โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€โ”€
...
class ModelTrainer:
    ...
    def evaluate(self, X: pd.DataFrame, y: pd.Series,
        ...
        preds_veto = ((p_dnn >= thr) & (p_xgb >= thr)).astype(int)
        ...
        log.info(f"  [Veto ens.] ROC-AUC: {roc_auc_score(yv, avg_p):.4f}")
        log.info(f"\n{classification_report(yv, preds_veto, digits=3)}")
        log.info(f"  Confusion Matrix (veto):\n{confusion_matrix(yv, preds_veto)}")
...
class TradingSimulator:
    ...
    Bar-by-bar simulation implementing:
        ...
        โ’ DNN veto gate (proba_dnn > threshold)
        โ’ XGB veto gate (proba_xgb > threshold)
    ...
    def run(self, df: pd.DataFrame,
        ...
        for i in range(len(df)):
            ...
            if open_trade is None and not np.isnan(atr) and atr > 0:
                # All three gates must pass โ€” STRICT VETO
                if (trends[i] == 1 and          # Gate 1: macro trend
                    p_dnn > thr and              # Gate 2: DNN confidence
                    p_xgb > thr):                # Gate 3: XGB confidence
    ...
    def compute_objective(results: pd.DataFrame) -> float:
        ...
        Metric: Profit Factor  (if net profit > 0, else penalised net profit)

        Why Profit Factor over raw P&L?
        โ€ข Scale-invariant: does not reward simply trading more.
        โ€ข Accounts for loss magnitude, not just win rate.
        โ€ข Penalising negative net-profit trials with their net PnL (a large
          negative number) creates a smooth gradient for the TPE sampler โ€”
          it can distinguish "lost a little" from "lost a lot" among bad trials
          instead of collapsing them all to โ’1e6.
        """
        if results.empty:
            return -1_000.0        # Heavy penalty: no trades at all

        net_pnl = results["pnl_usd"].sum()
        wins    = results[results["pnl_usd"] > 0]["pnl_usd"].sum()
        losses  = abs(results[results["pnl_usd"] <= 0]["pnl_usd"].sum())

        if net_pnl <= 0:
            # Return raw negative P&L (magnitude matters for TPE)
            return float(net_pnl)

        # Positive net profit โ’ use Profit Factor as primary metric
        pf = wins / (losses + 1e-9)
        return float(pf)
...
class OptunaOptimiser:
    ...
    Phase 2 โ€” Optuna study (n_trials)
        Each trial:
          ...
          e. Returns objective score (Profit Factor or penalised net P&L).
    ...
    def run_optimization(self) -> Dict:
        ...
        log.info("  Objective metric    : Profit Factor (penalised if โค 0)")
        log.info("โ•" * 62)
    ...
    def run_final(self) -> pd.DataFrame:
        ...
        n_veto = ((p_dnn > self.cfg.signal_threshold) &
                  (p_xgb > self.cfg.signal_threshold)).sum()
        log.info(f"  Signals โ’ DNN:{n_dnn}  XGB:{n_xgb}  Veto-agreed:{n_veto}")
